...
function join(parts) {
	for(var i = 0; i < parts.length; i++) {
		var part = parts[i];
		if(part.startsWith('/')) part = part.substr(1, part.length - 1);
...
window.$docsify.plugins.push(function(hook, vm) {
	...
	hook.beforeEach(function (html, next) {
		var repo = vm.config.tink.repo || vm.config.repo || window.location.pathname.substr(1); // strip the `/`
		var path = vm.route.file;
		var branch = vm.config.tink.branch || 'gh-pages';
		
		if(repo == 'haxetink') repo = 'haxetink/haxetink.github.io';
		
		var url = join(['https://github.com', repo, 'blob', branch, path]);
		var footer = ['[Edit this page](' + url + ')\n'];
